...
public sealed partial class ConsoleWindow
{
    ...
    private const string ListId = "##pvplb_log_list";
    private const string MenuId = "##pvplb_log_menu";
    private const string TimeSample = "00:00:00.000";
    private const string MultilineMark = " …";
    private const string RepeatMark = "×";
    ...
    private void DrawList(float height)
    {
        ...
        Paint.Surface(ImGui.GetWindowDrawList(), origin, origin + new Vector2(width, height), Styling.CardRounding * scale,
            Styling.WithAlpha(Styling.CardBg, 0.7f), Styling.WithAlpha(Styling.BorderDim, 0.5f));
    ...
    private void DrawRows()
    {
        ...
        for (var index = first; index < last; index++)
        {
            ...
            DrawRow(drawList, index, rowMin, new Vector2(innerWidth, rowHeight), columns, index == hoveredIndex);
        }

        DrawJumpPill(visibleHeight, innerWidth);
    ...
    private Columns MeasureColumns(float scale)
    {
        ...
        var sourceWidth = MathF.Min(sourceColumn, MaxSourceColumn * scale);
        var gap = ColumnGap * scale;
        return new Columns(time, tag, sourceWidth, RowPadX * scale + time + gap + tag + gap + sourceWidth + gap);
    }
    ...
    private void DrawRow(ImDrawListPtr drawList, int index, Vector2 rowMin, Vector2 rowSize, Columns columns, bool hovered)
    {
        var scale = ImGuiHelpers.GlobalScale;
        var line = view[index];
        ...
        if (selected)
        {
            Paint.Fill(drawList, rowMin, rowMax, Styling.WithAlpha(Styling.AccentRed, hovered ? 0.22f : 0.16f), rounding);
            Paint.Fill(drawList, rowMin, new Vector2(rowMin.X + SelectionBarWidth * scale, rowMax.Y), Styling.AccentRed, rounding);
        }
        else if (hovered)
        {
            Paint.Fill(drawList, rowMin, rowMax, Styling.WithAlpha(Styling.CardBgHover, 0.6f), rounding);
        }
        else if (line.Level == RunLogLevel.Error)
        {
            Paint.Fill(drawList, rowMin, rowMax, Styling.WithAlpha(Styling.AccentRed, 0.07f), rounding);
        }
        else if (line.Level == RunLogLevel.Warning)
        {
            Paint.Fill(drawList, rowMin, rowMax, Styling.WithAlpha(Styling.AccentAmber, 0.05f), rounding);
        }
        else if ((index & 1) == 1)
        {
            Paint.Fill(drawList, rowMin, rowMax, Styling.WithAlpha(Styling.CardBgSoft, 0.3f), rounding);
        }
        ...
        DrawTag(drawList, line.Level, new Vector2(x, rowMin.Y + RowPadY * scale * 0.5f), new Vector2(columns.Tag, rowSize.Y - RowPadY * scale));
        x += columns.Tag + gap;

        drawList.PushClipRect(new Vector2(x, rowMin.Y), new Vector2(x + columns.Source, rowMax.Y), true);
        ...
        DrawTrailing(drawList, line, text.RepeatLabel, rowMax, rowSize.Y, hovered);
    }

    private static void DrawTag(ImDrawListPtr drawList, RunLogLevel level, Vector2 min, Vector2 size)
    {
        var accent = LevelColor(level);
        Paint.Fill(drawList, min, min + size, Styling.WithAlpha(accent, 0.14f), size.Y * 0.5f);
        TextDraw.Middle(tagLabels[(int)level], min, min + size, Styling.Lighten(accent, 0.15f));
    }

    private static float TrailingWidth(in RunLogLine line, float scale)
    {
        var width = 0f;
        if (line.Repeat > 1)
        {
    ...
    private static float RepeatPillWidth(int repeat, float scale)
    {
        ...
        return TextDraw.Measure(RepeatMark).X + TextDraw.Measure("0").X * digits + TagPadX * 2f * scale;
    }

    private static void DrawTrailing(ImDrawListPtr drawList, in RunLogLine line, string? repeatLabel, Vector2 rowMax, float rowHeight, bool hovered)
    {
        var scale = ImGuiHelpers.GlobalScale;
        ...
        if (repeatLabel is not null)
        {
            ...
            Paint.Fill(drawList, min, max, Styling.WithAlpha(Styling.CardBorderDim, 0.9f), (max.Y - min.Y) * 0.5f);
            TextDraw.Middle(repeatLabel, min, max, Styling.TextSecondary);
            if (hovered && ImGui.IsMouseHoveringRect(min, max))
            {
                Tooltip.Show(string.Format(CultureInfo.InvariantCulture, RepeatedFormat, line.Repeat));
            }
    ...
    private ref RowText RowTextFor(in RunLogLine line, float messageWidth)
    {
        ...
        cached.RepeatLabel = line.Repeat > 1 ? string.Concat(RepeatMark, line.Repeat.ToString(CultureInfo.InvariantCulture)) : null;
        return ref cached;
    }

    private void DrawJumpPill(float visibleHeight, float innerWidth)
    {
        if (follow)
        {
            return;
        ...
        var scale = ImGuiHelpers.GlobalScale;
        var label = newWhileDetached > 0 ? newLinesText.For(newWhileDetached) : JumpLatestText;
        ...
        if (PillButton.Draw("##pvplb_log_jump", label, Styling.AccentRed, PillButton.Emphasis.Filled, FontAwesomeIcon.ArrowDown))
        {
            follow = true;
            newWhileDetached = 0;
        }
    ...
    private void DrawContextMenu()
    {
        ...
        if (ImGui.MenuItem(CopyLineText))
        {
            CopyRange(contextIndex, contextIndex);
        }

        var (low, high) = SelectionIndices();
        if (high > low && ImGui.MenuItem(string.Concat(CopySelectionText, " (", selectedText.For(high - low + 1), ")")))
        {
            CopyRange(low, high);
        }

        if (ImGui.MenuItem(CopyToEndText))
        {
            CopyRange(contextIndex, viewCount - 1);
        }

        ImGui.Separator();
        if (ImGui.MenuItem(string.Format(CultureInfo.InvariantCulture, OnlySourceFormat, line.Source), false, source is null))
        {
            source = line.Source;
            ClearSelection();
        }
    ...
    private void DrawEmpty()
    {
        ...
        var origin = ImGui.GetCursorScreenPos();
        var width = ImGui.GetContentRegionAvail().X;
        var padding = RowPadX * scale;
        var textWidth = MathF.Max(1f, width - padding * 2f);
        var y = origin.Y + padding;

        if (bufferedCount > 0)
        {
            TextDraw.Wrapped(NoMatchesText, new Vector2(origin.X + padding, y), textWidth, Styling.TextDim);
            y += TextDraw.MeasureWrapped(NoMatchesText, textWidth).Y + padding;
            ImGui.SetCursorScreenPos(new Vector2(origin.X + padding, y));
            if (PillButton.Draw("##pvplb_log_reset_empty", ResetFiltersText, Styling.AccentRed, PillButton.Emphasis.Tinted, FontAwesomeIcon.Undo))
            {
                ResetFilters();
            }
        ...
        TextDraw.Wrapped(EmptyText, new Vector2(origin.X + padding, y), textWidth, Styling.TextDim);
        y += TextDraw.MeasureWrapped(EmptyText, textWidth).Y + padding * 0.6f;
        TextDraw.Wrapped(FooterText, new Vector2(origin.X + padding, y), textWidth, Styling.TextMuted);
        y += TextDraw.MeasureWrapped(FooterText, textWidth).Y;
